...
- 详情页：左侧常驻播放器、双层时间轴和逐字稿证据栏，右侧可切换“会议纪要 / 章节脉络 / 屏幕内容 / 结论审计”。时间线上层是连续语义章节，下层是页面每次实际出现的区间；章节以讨论焦点、形成结果、后续动作、待确认和关键屏幕组成可点击脉络树。点击屏幕片段可查看完整 VL 解读并跳到该次出现时间。同一页重复出现会保留多个可跳转区间；空白、过渡、会议 UI 等低信息画面默认折叠但仍可查看，视觉展示不会被误写成会议决定。
