...
- 详情页：左侧常驻播放器、双层时间轴和逐字稿证据栏，右侧可切换“完整纪要 / 章节回顾 / 画面资料 / 质量验收”。时间线上层是连续语义章节，下层是页面每次实际出现的区间；点击章节可查看该时间段的摘要、画面、讨论、决定、行动项和待确认项，点击页面片段可查看完整 VL 解读并跳到该次出现时间。同一页重复出现会保留多个可跳转区间，不把视觉展示误写成会议决定。
