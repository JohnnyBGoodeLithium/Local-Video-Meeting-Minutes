...
- 详情页：左侧常驻播放器、双层时间轴和逐字稿证据栏，右侧可切换“会议纪要 / 章节脉络 / 屏幕内容 / 结论审计”。时间线上层是连续语义章节，下层是页面每次实际出现的区间；常规纪要不重复显示逐页详情，章节页沿纵向展示整场会议推进，屏幕页以缩略图和标题浏览 VL 资料。时间轴章节点击会播放并定位右侧，右侧章节点击只展开；页面片段仍可跳到对应出现时间。同一页重复出现会保留多个可跳转区间；空白、过渡、会议 UI 等低信息画面默认折叠但仍可查看，视觉展示不会被误写成会议决定。
