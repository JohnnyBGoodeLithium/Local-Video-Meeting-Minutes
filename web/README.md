...
- 逐字稿支持“原文 / 中文阅读 / 双语”三种显示。首次切换到中文或从纪要点击依据时，后台按连续轮次生成
  `transcript.translation.zh-CN.json`；翻译使用相邻发言、已确认名称、页面和关联结论消歧，但结论被明确视为
  低信任背景，不能向原话补事实。中文原话不重复显示，英文/混合轮次可随时展开原文；依据跳转会自动把
  关联轮次及相邻上下文展开为双语并插入优先队列。完整轮次按批次渐进显示，顶部展示进度条和完成数；
