...
check("前端装配入口使用可独立加载的原生 ES modules",
      ...
      and b'./modules/media-source.js?v=20260916p131' in app_js
...
if chrome:
    ...
    browser_build_present = "20260916p131" in browser.stdout
    ...
    workspace_browser = subprocess.run([
        str(Path(__file__).resolve().parent.parent.parent / ".venv/bin/python"),
        str(Path(__file__).resolve().parent / "chromium_workspace_ux_test.py"),
    ], capture_output=True, text=True, timeout=120, env=os.environ)
...
check("整篇重组保存为可切换 AI 视图，不覆盖标准纪要或会议脉络",
      ...
      and b'createMinutesTemplateDialog' in app_js
      and b'id="minutes-template-dialog"' in page
      and "保存为阅读版本".encode() in app_js
...
check("产品介绍页使用九段双语用户旅程与虚构演示",
      ...
      and b'/static/fluent-foundation.css?v=20260916p131' in product_page
