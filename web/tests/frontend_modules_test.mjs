...
const form = buildUploadFormData([new Blob(["video"])], {
  contentType: "media", noVl: true, ignoreTranscript: false, visualMode: "slides",
});
assert.equal(form.get("content_type"), "media");
assert.equal(form.get("visual_mode"), "slides");
...
}, "https://example.invalid/watch?v=synthetic", true, "slides");
assert.equal(request.path, "/api/import-url");
assert.equal(JSON.parse(request.options.body).no_vl, true);
assert.equal(JSON.parse(request.options.body).visual_mode, "slides");
