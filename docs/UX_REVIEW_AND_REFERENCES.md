...
- 走查日期：2026-08-11
- 范围：当前部署在 `127.0.0.1:8899` 的会议列表、详情阅读、播放时间线、逐字稿、质量验收、助手与导出入口；同时复核前端状态管理、响应式规则和 GitHub 同类项目。
- 隐私边界：界面截图只在本机 `/tmp` 临时生成并已删除；本文不记录真实逐字稿、纪要、人名或组织信息。
...
| 纪要阅读 | 正文宽度尚可，纪要/质量验收不离开会议 | 长纪要没有常驻目录；在线界面没有导出包已有的 audience/depth 视图；证据不完整的会议缺少醒目状态 | 增加轻量目录和“管理层/执行层 × 快速/精细”双开关；标题区显示 evidence `ready/partial/stale` |
...
| 导出分享 | MeetingPack v2 已有完整逐字稿、媒体跳转、证据、视图和图谱 | 三个导出按钮藏在菜单里，没有包大小、证据状态和收件人用途预检 | 合并为“分享/导出”向导：先选用途与媒体，再展示预计大小、evidence 状态、语言和包含内容 |
...
- 右侧顶部使用两个小开关选择“管理层/执行层”和“快速/精细”，下方仍可切回完整纪要；不要增加四个平铺页签。
...
- 恢复上次会议、语言、T ID/纪要标题和 pane ratio；刷新后 2 秒内回到可阅读状态。
- 标题区明确 processing/ready/stale 与 evidence ready/partial/stale；不再用一个可点击按钮掩盖证据不足。
- 紧凑/展开播放器、可拖动分栏、双层时间线；1366×768 下逐字稿可见区不少于 280px。
- 作业显示中文阶段与完成量，隐藏原始 slug；读取旧版本和后台重建可并存。
- 导出向导显示证据状态、媒体可用性和预计大小。

### P1：把 AI 交互做成可控过程

- 助手 SSE 流式输出、停止、阶段状态和稳定来源编号。
- 在线 audience/depth 视图与 MeetingPack 使用同一 `meeting-views/v1`，不维护两套选择规则。
