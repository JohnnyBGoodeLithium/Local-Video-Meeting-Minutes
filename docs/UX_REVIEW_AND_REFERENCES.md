...
- 右侧顶部使用两个小开关选择“管理层/执行层”和“快速/精细”，下方仍可切回会议纪要；不要增加四个 audience/depth 平铺页签。
...
2. 章节内用 evidence 构建“讨论焦点—形成结果—后续动作—待确认—关键屏幕”脉络树，叶子必须可回到逐字稿/页面来源；
3. 屏幕内容单独保留完整 VL 解释和每次出现区间，并标记核心/参考/低信息；默认隐藏低信息但允许查看，不能静默删除；
4. 当前脉络树表达单章内部关系。跨章节 Topic 图必须另建稳定主题节点与来源 linkage，不用页码、时间段或漂亮布局伪造语义关系；
