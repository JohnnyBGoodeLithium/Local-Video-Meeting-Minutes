...
逐字稿人工修正后的快速同步使用独立 `POST /api/meetings/{slug}/sync_minutes`，不改变失败恢复使用的标准 `regen_minutes`。`build_fast_sync_command()` 在排队前验证 `slides.json` 的每个逻辑页都有非空 `page_desc.json` 解读；视频会议通过 `--reuse-vl-cache-only --skip-topic-map` 严格禁止调用 VL，纯音频通过已有 `--skip-topic-map` 生成纪要。主作业只包含 `prepare → final_minutes`，并在 `job-progress/v2.available_outputs` 中保留已就绪逐字稿、人物导航和画面。纪要/evidence 成功后，`job_store` 才创建优先级更低的 Topic Map 后续作业；翻译与关键字沿既有派生任务执行，RAG 索引仍按 revision 懒重建。后续排队失败不回滚已发布纪要，缓存缺页则在写入前拒绝快速路径。
