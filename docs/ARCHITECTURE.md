...
`meeting_topic_map.py` 在纪要生成完成后使用本机 LLM 建立 `meeting-topic-map/v2` sidecar（revision 匹配的 v1 旧图仍判 ready）。它先把逐字稿按约十五分钟窗口做局部候选归纳，再把整场候选与 canonical claims 归并成 3–8 个一级 Topic 和类型化子节点。每个节点必须绑定有效 T/P/C ID；未知 ID 和无来源节点在代码层丢弃。相同 Topic 的非连续证据范围会保留在一个节点中。所有 map/reduce 请求使用 OpenAI-compatible `response_format=json_object` grammar 强制 JSON 语法，并限制标题/摘要长度、禁止复制逐字稿；响应只经过 `clean_reasoning_text`，保留 JSON 的括号、转义和 Markdown fence，绝不能使用会拆 `\boxed{}`/独占花括号的 VL 人读清洗器。grammar 只解决结构合法性，字段、引用和语义仍由 sanitizer 校验。reduce 若把局部候选的完整 turn 列表压成少数代表 ID，代码只在候选与最终 Topic 至少共享一个 turn 时继承该候选的其余引用；无共享锚点不猜归属，恢复数记录为 `candidate_turns_recovered`。一级 Topic 对 turn/claim 采用互斥主归属：模型把同一大段引用复制给两个 Topic 时，后者只保留未被前序 Topic 持有的来源，时间轴不再出现彼此覆盖的长条。局部模型明确列入 `uncovered_turn_ids` 的寒暄/等待可进入 `low_value` 分支；其余未归纳内容只有不超过 3 轮、45 秒发言且跨度不超过 120 秒的短漏挂片段才按邻近吸收，长未知区间保持时间轴缺口，绝不为追求覆盖率硬塞给最近标题。`stats.coverage` 按真正持有的原始 turn 区间计算，同时记录 `turn_coverage`、`unassigned_turns` 和 `overlap_turns_removed`。局部 map 和全局 reduce 返回不合法 JSON 时，只允许模型修复标点、引号与括号；修复仍失败时改用去掉全量 valid-id 列表的紧凑归并，再失败才投影局部候选。成功的局部窗口原子写入 checkpoint。sidecar 绑定逐字稿、纪要、页面和 VL revision，输入变化即标记 stale，旧节点不向前端暴露。
