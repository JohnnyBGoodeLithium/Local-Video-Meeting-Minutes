...
更新时间：2026-08-31（产品版本 v0.15.1；在线工作台构建号 20260831p109；提交号以 `git log -1` 为准）

## 当前状态速览（2026-08-31）

**最新交付（v0.15.1 / p109，feature branch `feat/v0.15.1-fast-sync`）**：

1. 逐字稿修正后的“快速同步纪要”与标准重生成分开：完整页面缓存才允许严格复用 VL，纪要/evidence 先发布，Topic Map、翻译和关键字后续排队；缓存缺页明确拒绝，不会漏画面冒充终稿。
2. `job-progress/v2` 的快速同步阶段只显示真实的准备与正式纪要，并把逐字稿、人物导航和已有画面标为可用。RAG 不伪装成固定后台阶段，仍按 revision 在下次使用时重建。
3. 永久一级“结论审计”改为条件式“核对关键结论”：只有重点待核对、已发现问题或 evidence 过期时出现入口；本地 evaluation 数据、全部证据和历史判断保持兼容，核对页中英文同批完成。
4. 后端快速同步为 `5be4ca5`，工作台与双语核对入口为 `b6d937d`；发布文档提交号以本轮最终 `git log` 为准。`make check` 全绿，隔离 `make smoke` 215/215，包含 Headless Chromium 的处理中、恢复、双语核对和现场资料完整旅程。

**上一交付（v0.15.0 / p108，feature branch `feat/v0.15-workspace-ux`）**：

1. `job-progress/v2` 成为新作业的结构化状态合同，稳定记录 route、用户阶段、单位进度、可用输出、ETA 范围、failure、recovery 和 attempt；旧作业继续通过明确标记的 legacy fallback 可读。实现链为 `53b8bec`。
2. 标题区使用唯一的处理/失败 banner，左栏任务卡只显示当前动作、可用结果和可信 ETA；处理详情、恢复预览、尝试历史和脱敏诊断进入统一非阻塞 side sheet。恢复默认复用最近检查点，降级结果明确边界。实现链为 `1e78cff`。
3. 会议页签改为“画面与资料 / Visuals & Materials”。现场资料先预览缩略图，再逐项设置位置；导入后可以看大图、改显示标题、定位/取消定位并安全删除。受保护原图、JPEG 阅读副本和 canonical sidecar 原子同步。实现链为 `3725c9e`。
4. 第四提交负责中英文、统一 sheet 视觉、产品介绍页、文档、p108、合成 before/after 截图和发布 tag；其提交号以当前 `git log -1` 为准。`make check` 全绿，隔离 `make smoke` 214/214，三条新 Headless Chromium 旅程均通过。
5. 本轮没有重写播放器、逐字稿、会议脉络、人物核对、导出或 canonical 会议 schema，也没有增加模型、Agent、RAG 或云端回退。

**部署边界**：当前本机服务仍以 main 的 v0.15.0/p108 为已部署基线。完成 feature branch、合入 main 和 tag 后，也必须先确认没有活跃作业并获得用户授权，才能重启到 v0.15.1/p109；未部署前浏览器继续显示旧构建属于正常状态。

**已知限制**：ETA 在没有足够近期成功样本时只显示“正在估算”，不会伪造倒计时；legacy 作业只能提供保守估算，无法补齐从未记录的历史阶段耗时；现场资料本轮不自动分析、不支持时间轴拖动，EXIF 只作为建议；离线 Viewer 继续只读，资料和人物修正需在在线工作台完成后重新导出。

**上一交付（v0.14.2 / p107，实现提交 `ac55ae9`）**：渐进式人物核对完整支持中英双语；录屏 Viewer 可独立选择无媒体、压缩音频或压缩视频；导入按钮移除重复加号。验证为 `make check` 与隔离 smoke 211/211。
...
**最新产品交付（v0.13.0 / p104，实现提交 `38eb1d6`）**：

1. 工作台“更多 → 发布到知识库”直接把当前会议/媒体发布到管理员 allowlist 中的 WeKnora 目标。会议默认轻量文字，媒体有关键画面时默认图文，含现场照片的会议也推荐图文；用户仍可在预检中改选。
2. `KnowledgeSink` 是 provider-neutral 合同；WeKnora adapter 用正文 revision 保证重复点击不复制。文本原位更新，图文先建新版本再删旧文档；`meeting.knowledge-publications.json` 只存目标、文档 ID、profile、revision 与状态，不存正文或 key。
3. 现场白板/纸面/黑板照片可按 EXIF、当前播放位置或未定位方式补入。受保护原图、JPEG 阅读副本、Web 时间线、Viewer、MeetingPack 与 KB 图文投影同步；照片不被当作独立决定证据。
4. 实现已在功能分支推送；隔离 smoke 206/206。发布文档提交后需再次运行 `make check` 的发布门禁，合入 main、推 tag 并在零活跃作业时重启 Web 才会启用新 Python 路由。

**最新产品交付（v0.12.0 / p103，实现提交 `c676f20`）**：

1. `/product` 已从旧版单一会议摘要叙事更新为当前能力：Meeting Identity、Evidence、Knowledge 三个核心，会议/媒体双入口，Teams/公开链接，渐进处理，事实层与自然语言重组，MeetingPack/KB/WeKnora 交接和统一内存资源护栏。
2. 产品介绍页以中文 DOM 作为无脚本可读基线，195 个英文文案键由同一页面投影；语言开关复用工作台 `meeting-minutes:workspace:v1` 偏好，并同步 `lang`、标题、描述和无障碍名称。
3. 页面接入共享 `fluent-foundation.css`，叙事配色收敛为 `--product*` 语义角色。`product_intro_test.py` 强制检查中英键一致、token 可解析，以及页面内容版本与 `VERSION` 的 major.minor 相同；以后 minor/major 发布不能漏掉介绍页复核。
4. 产品版本为 v0.12.0，在线缓存构建号为 p103。`make check` 全绿，隔离 `make smoke` 200/200；Headless Chromium 已确认工作台与产品页 ES module 均完整启动，无未捕获异常。实现提交为 `c676f20`，本发布以 annotated tag `v0.12.0` 标记。

**今日交付链**（主干 main）：p85 关键字索引+contentpack（7017381）→ p86 content_type 列表分离（62de2a9）→ p87 media 镜头检测（99c88de）→ p88 KB 导出 profile+时间码深链（275677d）→ p89 媒体版纪要 prompt（374f0ee）→ p90 屏幕列表滚动修复（76eb9f9）→ p91 口播帧二次合并（28ca26a）→ p92 疑难页 27B 复核+媒体浏览（7d58e68）→ p94 长口播时间修复、媒体自适应导航与图文 KB HTML（f228075）→ p95 公开视频链接、来源信息和内容分流任务卡（3795865）→ p96 前端模块第一阶段（2ee41a1）→ p97 内容库与核听导航模块（80d8827）→ p98 逐字稿与导出规则模块（b1c49db）→ p99 纪要视图与证据规则模块（3101d89）→ p100 逐字稿 DOM renderer（18b2529）→ p101 纪要 DOM renderer（b6fed79）。产品版本仍为 v0.11.0。

**最新工程交付（v0.11.1 / p102，实现提交 `d0d9055`）**：

1. `meeting_core.resource_policy` 统一处理文本请求、ASR/说话人、VL 和 120B 的内存准入；默认 32/24/8 GiB 三线、健康双模型、重阶段单模型、精修独占，资源等待进入作业阶段并可恢复。
2. WeKnora 纳入正式产品闭环：工作台可配置顶栏入口，仓库提供健康检查、固定版本/单并发配置、Compose 与资源守护 systemd 模板，以及 `docs/WEKNORA_INTEGRATION.md` 的完整移交和验收清单。
3. 本机部署同步设置 WeKnora 模型增强单并发、Meeting Web 知识库入口和资源阈值；真实凭据、数据库和文档不进入 Git。
4. 在线缓存构建号为 p102，产品版本 v0.11.1；`make check` 与 `make smoke`（199 项）通过，本机 Meeting Minutes、WeKnora、llama-router 和资源守护健康。

**最新工程交付（v0.11.0 / p101，已验证并提交，待本段溯源提交推送）**：

1. `minutes-view.js` 接管标准/AI 纪要阅读节点、语音草稿/翻译状态、待核实行动候选、标题锚点、证据时间文案与点击接线；标准纪要和 AI 视图的数据选择仍由 `minutes.js` 规则与入口 controller 完成。
2. renderer 只接收显式 presentation/evidence/translation 数据与 callback，不读取全局 `state`、不调用 API、不控制媒体；证据抽屉、AI 修改、保存、撤销、恢复上一版和 revision 校验仍留在 `app.js`。
3. Node 模块合同与在线 Headless Chromium 回归覆盖新模块；浏览器验收明确要求逐字稿 `turn-0` 和纪要 `minutes-heading-0` 同时实际出现。`make check` 全绿，隔离 smoke 198/198。
4. 实现提交 `b6fed79`。产品版本仍为 v0.11.0，在线缓存构建号为 p101；当前 8899 服务未运行，未擅自启动或重启。

**上一工程交付（v0.11.0 / p100，已验证、提交并推送）**：

1. `transcript-view.js` 接管逐字稿节点创建、长发言续段、译文/原文切换、待复核态、声纹/拆分选择、引用/修正入口和重渲染滚动锚恢复。
2. renderer 只接收显式逐字稿/翻译/复核状态、格式函数和行为 callback，不读取全局 `state`、不调用 API、不直接控制媒体；`app.js` 继续负责状态、播放、Focus、搜索、写入和重新装配。
3. Node 模块合同与在线 Headless Chromium 回归均已覆盖新模块；浏览器验收明确要求实际出现 `turn-0`，不是只验证页面未报错。`make check` 全绿，隔离 smoke 198/198。
4. 实现提交 `18b2529`，溯源提交 `cf03814`，均已推送。产品版本仍为 v0.11.0，在线缓存构建号为 p100。

**上一工程交付（v0.11.0 / p99，已验证、提交并推送）**：

1. `minutes.js` 接管标准/AI 纪要视图选择与失效回退、草稿/终稿/译文/证据能力状态、时间点到结论映射、行动项查找、证据轮次/页面/首个跳转时间和四个内容 Tab 的可用性。
2. 纪要 HTML、证据抽屉、AI 修改、保存/撤销/恢复和滚动副作用仍在 `app.js`，所以标准纪要保护、revision 冲突和现有交互不变。
3. 前端共有八个无 DOM 原生规则模块。独立 Node 合成回归和在线 Headless Chromium 模块图启动均通过；`make check` 全绿，隔离 smoke 198/198。
4. 实现提交 `3101d89`，溯源提交 `ccdbc3d`，均已推送。产品版本仍为 v0.11.0，在线缓存构建号为 p99；静态文件由现有服务即时读取，无需中断任务重启。

**上一工程交付（v0.11.0 / p98，已验证、提交并推送）**：

1. `transcript.js` 接管逐字稿命中查找、搜索游标循环、长轮次显示分段、分段核听时间计算和待复核索引；只改变代码归属，不改变长发言“同一发言 · N/M”或上一/下一段交互。
2. `export.js` 接管 full/kb/kb-html 与 none/audio/video 的白名单、体积格式和 30MB 提示判断，以及单场/多内容导出 URL 编码；弹窗 DOM、API 和下载点击仍在 `app.js`。
3. 前端当前共有七个无 DOM 原生模块。独立 Node 合成回归和在线 Headless Chromium 模块图启动均通过；`make check` 全绿，隔离 smoke 198/198。
4. 实现提交 `b1c49db`，溯源提交 `2ca779e`，均已推送。产品版本仍为 v0.11.0，在线缓存构建号为 p98；静态文件由现有服务即时读取，无需中断任务重启。

**上一工程交付（v0.11.0 / p97，已验证、提交并推送）**：

1. `library.js` 接管内容库排序、类型/关键字筛选、初始内容选择和深链时间边界；`player-navigation.js` 接管核听单元构造、当前段定位、按说话人筛选及上一/下一段计算。
2. DOM、滚动、`currentTime` 和 `play()/pause()` 副作用继续留在 `app.js`，所以内容库、说话人核听、深链和 Viewer 的用户行为不变；没有复制 Meeting/Media 页面。
3. 两个新模块已加入独立 Node 合成回归和在线 Headless Chromium 模块图启动回归。`make check` 全绿，隔离 smoke 198/198。
4. 实现提交 `80d8827`，溯源提交 `5ce7a08`，均已推送。产品版本仍为 v0.11.0，在线缓存构建号为 p97；静态文件由现有服务即时读取，无需中断任务重启。

**上一工程交付（v0.11.0 / p96，已验证、提交并推送）**：

1. 在线工作台从经典脚本切为无构建原生 ES module 入口；第一批抽出 `media-source.js`、`imports.js` 和 `jobs.js`，分别承载来源/内容类型投影、上传与链接请求构造、任务面板筛选规则。
2. 新模块不持有 DOM 或全局状态，`app.js` 仍负责页面装配和现有跨域状态；Meeting/Media 的 API、canonical 资产、Viewer 和用户旅程不变。这是渐进拆分，不是框架重写，也没有复制 Video App。
3. `frontend_modules_test.mjs` 用纯合成数据检查模块合同；隔离 Web smoke 新增 Headless Chromium 在线模块启动，验证模块路径/MIME、构建号、会议加载和无未捕获错误。`make check` 全绿，smoke 198/198。
4. 实现提交 `2ee41a1`，溯源提交 `84dc95b`，均已推送。本次只有工程边界和缓存构建号变化，不升产品版本；静态文件由现有服务即时读取，无需为本批强制重启处理中的任务。
...
3. Topic Map 对 media 附加 `media-navigation/v1`：单人口播展示议题+叙事作用，访谈展示议题+人物，混合视频展示三条；Web 与 Viewer 共享同一投影。
...
7. 全量 `make check` 与隔离 smoke 191/191 通过；服务在零活跃任务时安全重启，p94 健康。实现 `f228075`、功能溯源 `f82f8b5` 已推送。产品版本在文档收口时升为 v0.10.1。
...
- KB 视觉职责：会议侧 VL 文字是主语义，WeKnora VLM 默认关闭；只有要补读未文字化的截图字段时才开启。HTML/Markdown 的 ASR 关闭。若已开始多模态解析，取消该文档处理后在知识库设置或上传确认页关闭开关，再重新解析。
- 已确认本机 WeKnora docreader 能保留 HTML 的正文、图片与链接；仍需在最终产品界面验收“答案引用是否渲染成可点击深链”和 VLM 开/关 A/B 的实际问答收益。
- Git 纪律：验证全绿才推 main；发布打 tag；每批提交后回写 PRODUCT_FUNCTIONS Git 号。
- 待办队列：①按变更频率继续评估证据抽屉或视觉/脉络 renderer 的边界收益，不机械拆文件 ②公开链接用真实站点验收并在 LAN 前隔离下载 worker ③WeKnora API 直推 ④场内 AI 对话收缩为修正助手 ⑤知识库引用深链与 VLM 开/关 A/B 产品验收。
- 隐私红线与验证基线见"当前基线"段；真实会议正文不读，测试全用合成数据。

## 当前基线

- 仓库：`<repository-root>`，分支 main。
...
- 隐私红线（详见 AGENTS.md）：不读真实会议正文，只看元数据/结构；上次已获用户授权诊断 脱敏长会议 会议标题形态，新任务需重新授权。

## 当前批次：媒体疑难页 27B 复核 + 媒体浏览投影

- 两级 VL：媒体镜头仍由 MiMo 全量解释；仅 evidence 角色中带密集表格、复杂图表、跑分/规格/数值等信号，或轻量模型明确报告不确定的页面，限量交给 `MEETING_VL_REVIEW_MODEL` + `MEETING_VL_REVIEW_MMPROJ`。口播、空镜、普通演示不升级；复核失败保留 MiMo 结果。
- 工程护栏：复核端点必须校验实际模型身份，不能把占用默认端口的 embedding/reranker 误当 27B；端口冲突自动分配临时端口。源视频已清理时优先使用 `slides/full_N.jpg` 分析帧，仍可复核；已复核页命中缓存时不加载 27B。`page_desc.json` 记录主力/复核模型和页码，evidence generation 记录复核数量与失败数。
- 媒体 UI：工作台和 Viewer 对 media 使用“论证脉络 / 分析纪要 / 画面解析 / 依据审计”；画面按 Topic Map 的 `page_ids` 分组，可筛选证据/演示/铺垫/过渡，talking_head 折叠成出现次数与可跳转时间列表。导出 payload 显式携带 `content_type` 和同源结构字段，会议视图不变。
- 验证：真实媒体结构验收为 80/80 页、疑难页 6/6、复核失败 0、30 条 evidence claims；合成 `make check` 全绿、隔离 smoke 189/189、Headless Chromium Viewer 启动通过、导出 ZIP 96 项无损坏。实际 Topic Map 只有一个一级论点的原因是逐字稿只有一个超长 turn，已列为独立后续任务。
...
- 新增 `docs/PROCESSING_GUIDE.md` 面向非技术读者解释处理阶段、模型分工、状态和 ASR 术语边界。领域化选词仍是建议方案；已上线的定点复核 v1 只覆盖确认词表中的已知混淆，不等价于通用声学低置信检测。Context Pack 仍在 ASR 开始前构建一次，跨会候选池尚未按 不同业务领域 分域。
