...
当前发布基线为 `v0.13.1`（2026-08-28，Web p104）：会议与媒体可从工作台按各自知识结构一键
发布到 WeKnora，并按内容 revision 幂等更新；现场白板/纸面照片可用 EXIF、当前播放位置或未定位
方式补入 Web、Viewer、MeetingPack 和知识投影。照片是补充资料而不是独立决定证据；知识库 API
key 始终留在服务端。产品介绍以同一 DOM 提供完整中英双语阅读。
