...
当前发布基线为 `v0.14.1`（2026-08-31，Web p106）：说话人身份确认使用轻量身份卡，混声修复
按“手选少量样例 → 保守预览 → 多组分别确认 → 就近撤销”渐进展开，默认不自动扩展到相似片段，
已人工确认的身份仍由服务端保护。普通导出收束为离线 Viewer 与 AI / 知识库 Pack；既有 KB profile
和 WeKnora 发布接口保持兼容。产品介绍继续以同一 DOM 提供完整中英双语阅读。
