...
当前发布基线为 `v0.11.1`（2026-08-27，Web p102）：除会议/媒体共享内核与 KB 导出外，
WeKnora 已作为正式知识消费下游进入顶栏、部署模板、健康检查和交接文档；统一内存护栏允许健康时
双文本模型驻留，并在 ASR/说话人/VL/120B 等重阶段自动收缩和等待。知识库图文版保留图片，但
外部 VLM 仍是可选消费能力，不属于 mandatory pipeline。
