...
输入：Teams 导出的 .mp4(含屏幕共享) + Teams .vtt/.docx 逐字稿(自带姓名)。

流程：
    1. ffmpeg 抽 16k 单声道音轨 → 会议目录/audio.wav
    2. pyannote 本地分离(含每类聚类的声纹)
    3. 解析 Teams 逐字稿具名 turns；把分离聚类对齐到姓名标签：
...
用法：
    bin/teams_minutes.py meeting.mp4 meeting.docx
...
def align(transcript_cues, dia_turns):
    """聚类 ↔ Teams 姓名标签对齐。返回 (final_cues, stats, lab2final)。
...
def main() -> int:
    ...
    ap.add_argument("transcript", type=Path, help="Teams 导出的 .vtt 或 .docx 逐字稿")
    ...
    if not args.mp4.is_file() or not args.transcript.is_file():
        ...
        return 1
    if args.transcript.suffix.lower() not in {".vtt", ".docx"}:
        print("Teams 逐字稿只支持 .vtt 或 .docx", file=sys.stderr)
        return 1
    ...
    source_transcript = materialize_source(
        original_transcript, mdir / f"source.{transcript_format}")
    slug = mdir.name
    t_all = time.time()
    ...
    duration = max((end for _, end, _ in dia_turns), default=None)
    try:
        cues = parse_transcript(source_transcript, duration=duration)
    except TranscriptFormatError as exc:
        print(f"逐字稿解析失败: {exc}", file=sys.stderr)
        return 2
    ...
    source_meta = {
        ...
        transcript_format: str(source_transcript),
        f"original_{transcript_format}": str(original_transcript),
    }
    (mdir / "source.json").write_text(json.dumps(
        source_meta, ensure_ascii=False, indent=1), encoding="utf-8")
