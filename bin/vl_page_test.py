...
# media 镜头页（slides.json 里 shot:true）使用媒体向口径：帧的价值看它是否
# 承载论证信息，而不是是否像一页可复用的会议材料。
MEDIA_PROMPT = (
    "这是公开评测/上手视频中的一个镜头代表帧。请输出 JSON："
    "{\"type\":\"镜头类型(证据帧/演示帧/铺垫帧/过渡帧/其他)\","
    "\"title\":\"画面主题(画面里有标题文字就照抄,没有就写空字符串)\","
    "\"summary\":\"一句话说明这个镜头在展示什么(不超过50字)\"}。"
    "只输出 JSON，不要输出其它内容。"
)

MEDIA_DETAIL_PROMPT = (
    "这是公开评测/上手视频中的一个镜头代表帧。请详细解读这一帧，用 Markdown 输出：\n"
    "## 标题\n这一帧画面的主题(画面里有标题文字就照抄,没有就一句话概括)\n"
    "## 论证角色\n只写以下一个值：evidence / demo / context / transition / blank\n"
    "evidence=证据帧(规格表、对比图、跑分屏、价格页等支撑论点的材料)；"
    "demo=演示帧(真机展示、实际操作、效果演示)；"
    "context=铺垫/背景帧(人脸特写、纯讲解画面、场景空镜)；transition=过渡帧；blank=黑屏/空白\n"
    "## 信息价值\n只写 high / medium / low，再用一句话解释。high 表示这一帧承载了论证信息"
    "(规格、数据、对比、跑分、关键演示结果)；medium 表示演示过程、讲解画面等辅助理解的内容；"
    "low 表示纯铺垫、过渡、空镜或没有实质信息\n"
    "## 页面内容\n逐条列出画面里的要点、规格、数字、图表读数，表格列出关键的行和列"
    "——关键文字和数字尽量照抄，不要漏小字；人脸特写或空镜就写明画面主体是什么\n"
    "## 这帧想说明什么\n一两句话总结这一帧在整条视频论证中的作用\n"
    "只根据画面里真实存在的内容写，看不清的就说看不清，不要编。"
    "不要输出思考过程，不要输出 <think>、<analysis> 或前后解释。"
)
