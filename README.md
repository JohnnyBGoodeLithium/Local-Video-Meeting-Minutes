...
**当前产品版本：v0.15.1**（仓库根目录 `VERSION` 为单一真源）
...
3. 标题下方的处理状态会说明当前步骤、已经可用的结果和可信的剩余时间范围。逐字稿或语音草稿形成后即可先阅读、回听和导出核听快照；画面理解与正式纪要继续在后台完成。
4. 用户从会议脉络进入某个议题；时间操作改变播放位置，右侧节点选择只改变阅读 Focus。媒体内容会按实际形态自适应导航：单人口播显示“议题 + 叙事作用”，访谈显示“议题 + 人物”，混合内容同时保留两者。
5. 在纪要、逐字稿与画面之间核对证据，确认人员身份；必要时播放并修正单轮原语言文本。修正后使用“快速同步纪要”严格复用已有画面解读，先发布新的纪要与证据，再在后台更新脉络和翻译；画面缓存不完整时会明确要求走标准重生成。
6. “画面与资料”可补入白板、纸面、黑板或实物照片：先看缩略图，再逐张决定暂不定位、放到当前播放位置或采用拍摄时间建议；导入后可改名、重新定位、取消定位和删除。原图受保护，阅读副本进入 Web、Viewer、MeetingPack 和知识投影，但不会被单独冒充会议决定。
...
新作业把处理状态持久化为脱敏的 `job-progress/v2`。它只记录 route、阶段、单位进度、耗时、可用结果、失败分类和恢复范围，不包含标题、姓名、逐字稿、纪要、路径或原始 URL。历史作业继续通过明确标注的 legacy fallback 展示；恢复默认复用最近安全检查点，不会把“继续处理”伪装成从头重跑。详见 [处理与恢复指南](docs/PROCESSING_GUIDE.md)。
