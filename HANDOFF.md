...
更新时间：2026-08-25（产品版本 v0.10.0；在线工作台构建号 20260825p91；提交号以 `git log -1` 为准）
...
1. **玄戒 O3 重跑中**（后台 CLI：media 重抽 80 镜头→VL→媒体纪要 publish，约 1 小时；步骤命令在"媒体版纪要 prompt"批次段）。完成后验收：媒体章节结构、人脸帧折叠、干货帧完整。注意：重跑的抽取用的是口播合并**之前**的代码，若要应用 talking_head 合并需再重抽 slides。
2. **媒体 UI 一期（用户已批准，待动工）**：屏幕内容按论证环节分组折叠（topic map 的 topic 带 `page_ids`，数据就绪）；筛选 chips 换角色筛选（证据帧/演示帧/铺垫过渡/全部）；口播页折叠卡（口播·N 次·总时长→时间码列表）；media 默认第一眼=论证脉络；tab 文案切换（会议纪要→分析纪要、会议脉络→论证脉络、屏幕内容→画面解析）。二期：时间轴论证区段着色+骨干帧浮出。
...
- 待办队列：①玄戒验收+导出交付 ②媒体 UI 一期 ③WeKnora API 直推 ④meeting-dl CLI 正式化 ⑤场内 AI 对话收缩为修正助手。
