...
更新时间：2026-08-18（产品版本 v0.8.2；当前工作树构建号 20260818p53；提交号以 `git log -1` 为准）
...
- 会议脉络、会议纪要、屏幕内容和结论审计使用全宽浏览布局；逐字稿是独立 Tab，进入后才展开左侧播放器/章节与说话人时间轴、右侧大面积逐字稿。发言级上一段/重播/下一段与播放范围只在逐字稿核听区保留一份。
- 浏览状态播放继续时仅显示紧凑悬浮播放器；点击纪要依据、脉络显式时间或屏幕出现区间会进入逐字稿核听，不把普通节点选择误作 seek。搜索框固定在阅读区顶部，按当前 Tab 独立保存关键词与命中位置。
- MeetingPack 同步四入口与两态布局。修复节点详情在读取相关屏幕变量前发生 TDZ 的运行时错误；点击脉络节点恢复展开说明且不播放，只有时间按钮进入核听。当前前端构建 `20260818p53`。
