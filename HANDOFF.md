...
更新时间：2026-08-31（产品版本 v0.14.2；在线工作台构建号 20260831p107；提交号以 `git log -1` 为准）

## 当前状态速览（2026-08-31）

**最新交付（v0.14.2 / p107，实现提交 `ac55ae9`）**：

1. v0.14.1 的渐进式人物核对现已完整支持中英双语；身份卡只保留“稍后处理”一个退出动作，混声修复、保护/存疑、多组摘要、错误和就近撤销均跟随工作台语言。Headless Chromium 会实际切到英文并打开两层界面。
2. 离线 Viewer 的媒体选项按真实可用性独立列出；录屏会议可在不带媒体、压缩 AAC 音频和压缩视频间选择，视频存在不再隐藏音频。普通导出仍只有“离线 Viewer”和“AI / 知识库 Pack”两个消费意图。
3. 导入按钮只保留 Fluent Add 图标，不再叠加文本“+”。人物核对退出确认使用显式 `exitConfirmation` 状态，不解析本地化错误文本；新增/修改用户文案必须中英同批交付的规则已写入 `AGENTS.md`。
4. 实现提交 `ac55ae9` 已推送 main。验证：`make check` 全绿；隔离 `make smoke` 211/211，包含人物确认、撤销、混声预览/应用、英文界面和导出回归。
5. 本机 `meeting-minutes-web.service` 已在确认无会议处理进程后启动；健康端点返回 `version=0.14.2`、`active_jobs=0`。
