...
4. 实现提交 `2ee41a1`。本次只有工程边界和缓存构建号变化，不升产品版本；静态文件由现有服务即时读取，无需为本批强制重启处理中的任务。
...
- 待办队列：①继续抽离 `app.js` 的 library/player/transcript/minutes/export 边界 ②公开链接用真实站点验收并在 LAN 前隔离下载 worker ③WeKnora API 直推 ④场内 AI 对话收缩为修正助手 ⑤知识库引用深链与 VLM 开/关 A/B 产品验收。
