...
4. 实现提交 `b1c49db`。产品版本仍为 v0.11.0，在线缓存构建号为 p98；静态文件由现有服务即时读取，无需中断任务重启。
...
- 待办队列：①继续收敛 `app.js` 的逐字稿 DOM 渲染与 minutes 证据/编辑状态 ②公开链接用真实站点验收并在 LAN 前隔离下载 worker ③WeKnora API 直推 ④场内 AI 对话收缩为修正助手 ⑤知识库引用深链与 VLM 开/关 A/B 产品验收。
