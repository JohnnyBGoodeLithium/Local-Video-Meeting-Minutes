...
更新时间：2026-08-18（产品版本 v0.8.2；在线工作台构建号 20260818p58；提交号以 `git log -1` 为准）
...
- 当前真实服务仍由本批后端代码更新前启动，因有上传/VL 作业运行而未重启；p58 静态修复已实时生效，但最近导入/最近更新字段和安全重转写端点要在队列清空后的服务重启才正式上线。不要把前端选项出现误报为后端已部署。
