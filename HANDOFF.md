...
更新时间：2026-08-31（产品版本 v0.14.1；在线工作台构建号 20260831p106；提交号以 `git log -1` 为准）

## 当前状态速览（2026-08-31）

**最新交付（v0.14.1 / p106，实现提交 `12c74f8`）**：

1. 说话人 chip 默认打开轻量身份卡，最多三步完成整组身份确认；代表片段覆盖会议前/中/后。只有用户选择“这组里混入了其他人”才进入非阻塞人物核对侧栏，播放器和逐字稿上下文不丢失。
2. 高级核对先手选少量错误样例，再预览手选、相似建议、人工保护、证据不足和多个结果组。默认 `expand_similar=false`；每组可分别选已有人员、新建或保持待确认。应用前有段数/时长前后摘要，失败保留选择，成功后就近撤销。后端继续强制保护人工确认并复用原 revision/history 事务，不改 canonical 会议格式。
3. 普通导出只显示“离线 Viewer”和“AI / 知识库 Pack”；`kb` / `kb-html` API/CLI/知识库发布保持兼容。“更多”将重新处理、重新分类、历史撤销和管理入口折叠进维护区。
4. 实现边界为 `speaker-correction.js`（状态/预览/payload）+ `speaker-correction-view.js`（无 API DOM projection）+ `app.js` 装配。验证：`make check` 通过；隔离 `make smoke` 211/211，通过真实 Headless Chromium 身份确认、撤销、混声预览与应用旅程。

**已知限制**：代表片段优先级当前以时长与会议前/中/后分布为主，只有上游提供清晰度元数据时才能真正按音频质量排序；极短片段可明确改派给已有人员，但要自动寻找相似发言或创建可复用的新声音组仍需要足够音频证据；离线 Viewer 继续只读，身份修正必须在在线工作台完成后重新导出。
