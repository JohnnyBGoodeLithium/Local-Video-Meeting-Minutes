...
更新时间：2026-08-19（产品版本 v0.9.1；在线工作台构建号 20260819p68；提交号以 `git log -1` 为准）
...
- AI 栏的明确局部修改通过校验后写入标准纪要并提供一步撤销；整篇重组保存为 `minutes.views.json` 中的独立 AI 视图，绝不覆盖 `minutes.md`，标题栏始终可切回标准纪要。历史存在时显示“恢复上一版”，服务重启后也可恢复且先备份当前版本。结果卡保持紧凑，完整版本按需展开。后端返回禁用原始 HTML 的安全 Markdown，普通问答表格在窄栏正确渲染，旧版未格式化长表格默认折叠。标准纪要与 AI 视图都从结构化 action 重建待办表，编号/双语栏目名、半角/全角竖线及空 `action.text` + 非空右移字段均可自愈。在线构建号为 `20260819p68`。
