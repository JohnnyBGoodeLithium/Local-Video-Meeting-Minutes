...
更新时间：2026-08-18（产品版本 v0.8.2；在线工作台构建号 20260818p59；提交号以 `git log -1` 为准）
...
- 仅离线 MeetingPack 使用四入口与响应式工作台：首次脉络/纪要/屏幕仍全宽浏览；进入逐字稿或点击显式时间后，大屏同时显示播放证据、会议背景和全高逐字稿，中屏显示播放证据与当前工作区。在线工作台保持既有窗口比例。
- 导航与搜索移到工作台固定层，切换逐字稿不再改变 Tab 屏幕位置。大屏比例约 30/32/38，中间脉络栏达到 620px 时自动两列；逐字稿保持常驻右栏。节点选择不 seek，详情开头会进入背景栏可视区；显式时间才播放。
- Viewer 人物车道加粗至 16px、姓名区加宽，逐字稿整行使用稳定人物色。议题与人物使用不重叠的冷色/分类色调色板，并在总时间线明确标注“议题/人物”。
- 在线端与 Viewer 的自动跟随只在核听段落变化时居中当前段，避免停在底部和每帧滚动。长发言核听索引与播放规则保持不变。在线构建为 `20260818p59`。
