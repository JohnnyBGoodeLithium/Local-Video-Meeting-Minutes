...
更新时间：2026-08-19（产品版本 v0.9.1；在线工作台构建号 20260819p64；提交号以 `git log -1` 为准）
...
- AI 栏的明确修改/重组命令通过校验后直接写入，专用按钮只作为快捷提示；结果卡保持紧凑并提供撤销，完整写入版本按需展开。后端返回禁用原始 HTML 的安全 Markdown 渲染结果，前端不再展示 marker 或三个长文本滚动框。普通问答的长度截断会显式失败。
